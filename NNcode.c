#include<stdio.h>
 ...
 int main()
 {
	...
	printf("enter number of patient records for training \n");
	scanf("%d",&patterns);
    ...
    cho:
	printf("enter choice 1. train network 2. calculate risk \n");
	scanf("%d",&c);
	switch(c)
	{
	case 1:
	...
	printf("TARGET risk values of patients for training are \n");
	...
	{      if(epoch>=1)
		...
		printf("error is greater than threshold \n");
		if(epoch < 3000)
		{
		    ...
		    if(learningrate<1)
		    {
		      printf("MSG: since epochs are less ");
		    ...
		    else
		    {
			  ...
			  learningrate=learningrate-((epoch/10)*0.01);
		       ...
		       printf("(else)learningrate has been changed to %lf...\n",learningrate);
		    }
		}
		else
		{
...
void updateweights(double momentum)
 {
   printf("this is update function \n");
